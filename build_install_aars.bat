...
echo SayCV_MXE: Execute install jars.
cd ../sayJCMs
...
cd ../
set HOME=%cd%
rem set ORIGIN_HOME=%cd%
rem retain directory style
rem bash --login -i -c "find slf4j -name "*.jar" | xargs -I {} cp  --parents {} sayJCMs"
bash --login -i -c "find root-commands -name "*.aar" | xargs -I {} cp {} sayJCMs/android"

rem do not retain directory style
rem find . -path "./dwgfiles" -prune -o -name "*.dwg" | xargs -I {} cp {} dwgfiles
rem find . -path "./dwgfiles" -prune -o -name "*.dwg" | xargs cp -t dwgfiles 
...
find . -path "./dwgfiles" -prune -o -name "*.dwg" | xargs -I {} cp  --parents {} sayJCMs/slf4j
REM ##############################
REM End ...
REM ******************************
